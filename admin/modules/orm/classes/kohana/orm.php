...
class Kohana_ORM extends Model implements serializable {
        ...
        if ($id !== NULL) {
            if (is_array($id)) {
                foreach ($id as $column => $value) {
                    //IF IS NUMERIC, USES THE PRIMARY KEY
                    if(is_numeric($column)){
                        $column = $this->_primary_key[$column];
                    }
    ...
    public function values(array $values, array $expected = NULL) {
        ...
        if ($expected === NULL) {
            ...
            if(is_array($this->_primary_key)){
                foreach($this->_primary_key as $pri){
                    unset($values[$pri]);
                }
            }else{
    ...
    public function with($target_path) {
        ...
        } else if (isset($parent->_has_many[$target_alias])) {
    ...
    protected function _load_values(array $values) {
        //IF PRIMARY KEY IS ARRAY, ITS COMPOSITE KEY
        if(is_array($this->_primary_key)){
            //TAKE THE ARRAY COUNT TO CHECK IF EXISTS
            $cont = count($this->_primary_key);
            
            $qnt = 0;
            $valuesPK = array();
            foreach($this->_primary_key as $pri){
                if (array_key_exists($pri, $values)) {
                    if($values[$pri] != NULL){
                        $qnt++;
                        $valuesPK[$pri] = $values[$pri];
                    }
                }
            }
            ...
            if($qnt == $cont){
                ...
                // Store primary key
                $this->_primary_key_value = $valuesPK;
            }else {
                // Not loaded or valid
    ...
    public function create(Validation $validation = NULL) {
        ...
        //IF PRIMARY KEY IS ARRAY, CHECKS EVERY ONE
        if(is_array($this->_primary_key)){
            $total = count($this->_primary_key);
            $cont = 0;
            
            foreach($this->_primary_key as $valor){
                if (!array_key_exists($valor, $data) or $data[$valor] == 0) {
                    $cont++;
                }
            }
            ...
            if ($cont == $total) {
                ...
                foreach($this->_primary_key as $campo => $valor){
                    $this->_object[$valor] = $this->_primary_key_value[$campo] = $result[0];
                }
            } else {
                foreach($this->_primary_key as $campo => $valor){
                    $this->_primary_key_value[$campo] = $this->_object[$valor];
                }
            }
        }else{
            //ALTERAÇÃO PAULO 09/06/2014
            //COMO O ID VAI ESTAR PRESENTE NO ARRAY $DATA, TESTA SE O VALOR ESTÁ ZERADO
            if (!array_key_exists($this->_primary_key, $data) or $data[$this->_primary_key] == 0) {
                // Load the insert id as the primary key if it was left out
    ...
    public function update(Validation $validation = NULL) {
        ...
        //IF IS ARRAY, MOUNT IT
        if(is_array($id)){
            $db = DB::update($this->_table_name)->set($data);
    ...
    public function count_all() {
        ...
        //AO INVES DE COLOCAR DIRETO (QUE PARECE ESTAR DANDO CONFLITO COM INDICES DE ARRAY), FAZ UM FOREACH
        //$this->_db_pending += $selects;
        foreach ($selects as $sel) {
            $this->_db_pending[] = $sel;
        }
    ...
    //PAULO 11/06/2014
    //SETA AS COLUNAS NO SELECT
    public function setColumns($columns) {
        $this->_table_columns = $columns;
    }

    //CRIPTOGRAFA A SENHA
    public function criptografaSenha($valor) {
        return md5(hash("sha512", Cookie::$salt . addslashes($valor)));
    }

    //ARRUMA DATA PARA SALVAR
    public function arrumaData($valor) {
        return Controller_Index::ddmmaaaa_aaaammdd($valor);
    }

    //ACEITA APENAS OS VALORES "S" OU "N" (PARA VALOR SIM OU NÃO)
    public function valorSN($valor) {
    ...
    //ARRUMA VALORES FLOAT PARA SALVAR
