<section id="formulario">
    ...
    <div class="col-md-12">
        <div class="box box-info">
            <form action="<?php echo url::base() ?>blog/save" class="form-horizontal" id="formEdit" name="formEdit" method="post" enctype="multipart/form-data" >
        ...
        <!--FORMULARIO COM INFORMACOES-->
                <input type="hidden" id="BLO_ID" readonly name="BLO_ID" value="<?php echo $blog["BLO_ID"] ?>">
    
        <div class="form-group">
            <label for="BLO_TITULO" class="col-sm-2 control-label">Título *</label>
            <div class="col-sm-10">
                
                <input type="text" validar="texto"  class="form-control  " placeholder="Título" value="<?php if($blog) echo $blog["BLO_TITULO"] ?>" id="BLO_TITULO" name="BLO_TITULO" >
            </div>
        </div>
                            
        <div class="form-group">
            <label for="BLO_TEXTO" class="col-sm-2 control-label">Texto *</label>
            <div class="col-sm-10">
                <textarea  class="form-control ckeditor" placeholder="Texto" id="BLO_TEXTO" name="BLO_TEXTO">
                    <?php if($blog) echo $blog["BLO_TEXTO"] ?>
                </textarea>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-10">
                ...
                foreach($categorias as $cat){ 
                    $blogcategorias = ORM::factory('blogcategorias')->where('BLO_ID', '=', $blog["BLO_ID"])->where('CAT_ID', '=', $cat->CAT_ID)->find(); ?>
                    <input type="checkbox" value="<?php echo $cat->CAT_ID ?>" <?php if($blogcategorias->CAT_ID > 0) echo 'checked'; ?> id="CAT_ID_<?php echo $cat->CAT_ID ?>" name="CAT_ID[]"> <?php echo $cat->CAT_TITULO ?> &nbsp;&nbsp;&nbsp;
                <?php 
                } ?>
            </div>
        </div>
        ...
        <div class="form-group">
            <label for="USU_ID" class="col-sm-2 control-label">Usuário *</label>
            <div class="col-sm-10">
                <select class="form-control select2" id="USU_ID" name="USU_ID" validar="int" >
                    <?php foreach($usuarios as $usu){ ?>
                    <option value="<?php echo $usu->USU_ID ?>" <?php if($usu->USU_ID == (int)$blog["USU_ID"]) echo "selected"; ?>>
                        <?php echo $usu->USU_NOME ?></option>
                    <?php } ?>
                </select>
            </div>
        </div>
...
if ( typeof CKEDITOR == "undefined" ){
    ...
    var editorBLO_TEXTO = CKEDITOR.replace( "BLO_TEXTO" );
    CKFinder.setupCKEditor( editorBLO_TEXTO, "<?php echo url::base()?>js/ckfinder/" ) ;
}
// ]]>
</script>
